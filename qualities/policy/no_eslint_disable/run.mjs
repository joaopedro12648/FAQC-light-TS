#!/usr/bin/env node
/**
 * @file eslint-disable 全面禁止ポリシーのランナー
 * 備考: 特記事項なし
  ...
  while ((m = rx.exec(content)) != null) {
    ...
    for (let i = 0; i < lines.length; i += 1) {
      ...
      if (/^eslint-(?:disable|disable-next-line|disable-line|enable)\b/i.test(normalized)) {
        ...
        const textForOutput =
          // 条件式の評価: 原文の有無に応じて出力テキストを選択する
          display.length > 0 // 分岐根拠: 実際のコメント原文の有無
            ? display // 非空時は原文を採用
            : '/* eslint-* */'; // 空の場合は代表表記で指令を示す
...
function scanLineComments(content) {
  ...
  for (let i = 0; i < lines.length; i += 1) {
    ...
    if (idx < 0) continue; // 行コメントなし（解析対象外）
...
try {
  ...
  process.stderr.write(`[policy:no_eslint_disable] fatal: ${String((e?.message) || e)}\n`);
  process.exit(2);
}
