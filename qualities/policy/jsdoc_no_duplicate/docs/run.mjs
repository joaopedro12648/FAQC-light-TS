#!/usr/bin/env node
/**
 * @file JSDoc 重複防止ポリシーのランナー（docs ユニット実体）
 * 備考: qualities/policy/jsdoc_no_duplicate/docs/** を docs ユニットの代表ディレクトリとし、本ファイルに実装ロジックを集約する
 ...
 * - jsdoc_no_duplicate も SoT IGNORES を単一起源として利用し、個別 EXCLUDE を設けない
 */
...
function listFilesRecursive(dir) {
  ...
  while (stack.length) {
    ...
    for (const e of entries) {
      ...
      // 除外対象ディレクトリは走査から外す
      if (e.isDirectory()) {
        // SoT で定義された除外ディレクトリ名に一致する場合は配下の探索自体をスキップし、不要な検査コストを避ける
...
function intersectTags(a, b) {
  ...
  for (const t of a) {
    // もう一方の集合にも含まれるタグだけを共通タグとして採用し、重複説明の候補として記録する
...
try {
  ...
  process.stderr.write(`[policy:jsdoc_no_duplicate] fatal: ${msg}\n`);
  process.exit(2);
}
