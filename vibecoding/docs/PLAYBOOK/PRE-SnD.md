...
- 4) IMPL 承認フレーズの記録と提示（必須・1回だけ）:
  - Ready 遷移時に、対象 SnD front matter の `impl_approval_phrase` に IMPL 開始用フレーズ（例: `PHASE=IMPL 承認: SnD=<path>`）を記録する。
  - Ready 遷移直後に、`impl_approval_phrase` の値を用いた IMPL 開始用 4 行ブロック（要旨/別チャット可/ガイダンス/承認フレーズ）を必ず1回提示する（別チャットでも可）。
...
3. SnD 内の明記ポイント
   ...
   - ステータス運用/受け入れ条件: Ready 判定・受け入れ基準を具体的に列挙し、受け入れ条件を次の4カテゴリ（UI・UX可/不可、UI・UX以外の可/不可）で書き分けること：
     ...
     - 受け入れ条件（UI・UX以外）: 型安全性・ポリシー・セキュリティ・パフォーマンス等、品質ゲートや非機能に関する条件
     - 受け入れ不可条件（UI・UX以外）: CORSエラーや抑止コメント依存など、システムとして許容しない状態
   ...
   - （ブラウザ/DOM 系の SnD の追加必須事項・技術非依存）
     - 対象システムの「動作確認経路」を SnD に明示する：
       - ブラウザ/DOM + ES modules / TypeScript を扱う SnD の既定値は「開発サーバ要」とし、`file://` で `index.html` を直接開く運用は原則として想定しない。
       - コンソールプログラムや純CLIツールなど、ブラウザを介さない SnD では「開発サーバ不要」として明示し、その理由（入出力がすべてローカルプロセス内で完結する等）を簡潔に記載する。
     - 開発サーバの要否を明記する（既定: 要。純静的のみとする合理的理由がある場合は理由も記載）
     - スクリプト規約（役割名のみ定義）: dev（開発サーバ）/ build（本番ビルド）/ preview（静的プレビュー）を SnD の `context.outputTargets` と README に明示する（具体ツール名は任意）
     - CI 方針: 開発サーバはローカル開発専用。CI のチェックは lint/typecheck/test/build のみとし、dev/preview を使わない
     - 静的のみを選ぶ場合の受入条件: CORS/相対パス/アセット解決が実際に問題にならないことを事前検証し、SnD の受入条件へ引き上げる（例: ESM を使わない単一HTML/JS、あるいはCORS/モジュール解決に依存しない構成であることをSnD本文に明示する）
     - 代表的な NG 例を SnD 側で避ける:  
       - `file:///.../index.html` を直接開いたときに、ブラウザのコンソールで次のようなエラーが出る場合は「動作確認経路の設計ミス」として扱い、SnD の動作確認手順を修正すること（例）:  
         - `origin 'null' has been blocked by CORS policy`  
         - `Failed to load module script` / `Cannot load file:///.../*.js`  
       - 上記のようなエラーが出ている状態を「一時的な実装」として許容せず、SnD の「動作確認手順」と実際の起動方法（`npm run dev` 等）を一致させる。  
...
- 受け入れ条件節が「受け入れ条件（UI・UX）」「受け入れ不可条件（UI・UX）」「受け入れ条件（UI・UX以外）」「受け入れ不可条件（UI・UX以外）」の4カテゴリで整理されている
- 「品質ゲート（このSnDの定義）」が明記され、テスト観点が列挙
- 特段の理由がない限り、「CHECK ロール（`npm run --silent check`）の緑化」を受け入れ条件（UI・UX以外）の一部として含め、その実行結果を SnD の受け入れ条件節に明記する（現実的でない場合は理由と代替ゲートを明記する）
- PRE-COMMON によるコンテキスト更新が完了し、参照リンクが SnD に記載
- SnD作成完了後、未確定事項が存在しない場合はハッシュを取得し、Status 変更ルールに則りすぐに Status を Ready 化する
