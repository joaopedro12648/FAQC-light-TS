...
　- 背景: 本リポでは eslint のルールが非常に厳しく、複数ファイル作成・編集後に Lint ルールを把握しても、
    ...
    このルールに従う必要がある。そうでない場合、トークン過消費、タイムボックス超過、品質の低下を招き、タスク完了の達成が非常に困難なものになる。
  - 各ファイルを新規作成/編集して保存したら、次のファイルに着手する前にそのファイル単体を対象に ESLint を非対話で実行する（例: `eslint <file>`）。
  - エラー/警告が1件でも出た場合は、その場で修正してから続行する（`--max-warnings=0` 前提）。
  - エージェントは本ルールをブロッキング条件として扱い、失敗時は停止→修正→再実行後に続行する。
