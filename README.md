...
- 動作確認対象 LLM モデルは、GPT-5 / GPT-5 High / Sonnet 4.5 (Thinking) です。
- 実装完了時点では成果物に不具合が含まれる場合もあり、それは別途追加の「会話」で修正しています。
- SnD 作成から実装完了までに、モデルにもよりますが 2,000万トークン前後を必要とします。修正に関しては内容、追加要件次第ですがSnD～実装完了と同程度の場合が多い体感です。
...
- 別チャットで実装し、元チャットでレビュー
  ```
  （別チャット/IMPL）PHASE=IMPL 承認: SnD=vibecoding/var/SPEC-and-DESIGN/SnD-<YYYYMMDD>-<slug>.md
  実装が完了しました。品質ゲートを通過したことを報告してください。

  （元チャット/SnD）別チャットで上記 SnD の実装が完了しました。コードベースと SnD を根拠に差分の要点をレビューし、レビュー結果はこのチャットに出力してください。
  ```
